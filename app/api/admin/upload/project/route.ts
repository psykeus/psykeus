...
// ProcessedFile implements SortableFile for use with sortFilesByPriority
interface ProcessedFile extends SortableFile {
  buffer: Buffer;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const imageFiles = processedFiles.filter(f => isImageFile(f.name));
    const designFiles = processedFiles.filter(f => isDesignFile(f.name));

    // Sort all files by preview priority
    // sortFilesByPriority is imported from @/lib/file-types
    const sortedByPreview = sortFilesByPriority(processedFiles, PREVIEW_FILE_PRIORITY);
    const previewFile = sortedByPreview[0];

    // Sort design files by primary priority
    const sortedDesigns = sortFilesByPriority(designFiles, PRIMARY_FILE_PRIORITY);
    const primaryDesignFile = sortedDesigns[0] || previewFile;
    ...
    if (generateAiMetadata) {
      try {
        ...
        const isMultiFileProject = processedFiles.length > 1;
    ...
    for (let i = 0; i < processedFiles.length; i++) {
      ...
      const isPrimary = file.name === primaryFile.name;
    ...
    // Update design with primary_file_id and preview_path
    if (primaryFileId) {
      await supabase
        .from("designs")
        .update({
