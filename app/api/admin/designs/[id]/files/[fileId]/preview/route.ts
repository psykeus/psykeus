...
/**
 * GET /api/admin/designs/[id]/files/[fileId]/preview
 * Get a signed URL to preview a specific file in a design (admin only)
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  const { id: designId, fileId } = await params;
  ...
  if (NEEDS_GENERATED_PREVIEW.includes(fileType)) {
    ...
    try {
      ...
      if (previewResult.success && previewResult.buffer) {
        ...
        await supabase.storage.from("designs").upload(previewPath, previewResult.buffer, {
          contentType: "image/png",
          upsert: true,
        });

        // Get signed URL for the new preview
        const { data: newPreview } = await supabase.storage
          .from("designs")
          .createSignedUrl(previewPath, 3600);
