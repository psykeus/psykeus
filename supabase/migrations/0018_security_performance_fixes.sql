...
-- 1. PERFORMANCE: RLS policies re-evaluating auth.<function>() for each row
--    Fix: Wrap auth.uid()/auth.jwt() in (SELECT ...) for single evaluation
-- 2. SECURITY: Functions with mutable search_path
--    Fix: Add SET search_path = '' to all functions

-- ============================================================================
-- PART 1: Fix Helper Functions (Security + Performance)
-- ============================================================================

-- Fix is_admin() - add search_path and wrap auth.uid()
...
-- Fix is_super_admin() - add search_path and wrap auth.uid()
...
-- ============================================================================
-- PART 2: Fix RLS Policies - Users table
-- ============================================================================

-- Drop and recreate users policies with (SELECT auth.uid())
...
-- ============================================================================
-- PART 3: Fix RLS Policies - Downloads table
-- ============================================================================
...
-- ============================================================================
-- PART 4: Fix RLS Policies - User Favorites table
-- ============================================================================
...
-- ============================================================================
-- PART 5: Fix RLS Policies - Collections table
-- ============================================================================
...
-- ============================================================================
-- PART 6: Fix RLS Policies - Collection Items table
-- ============================================================================
...
-- ============================================================================
-- PART 7: Fix RLS Policies - User Sessions table
-- ============================================================================
...
-- ============================================================================
-- PART 8: Fix RLS Policies - User Access Tiers related tables (if they exist)
-- ============================================================================

-- user_tier_history
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_tier_history') THEN
    DROP POLICY IF EXISTS "Users can view own tier history" ON public.user_tier_history;
    EXECUTE 'CREATE POLICY "Users can view own tier history" ON public.user_tier_history
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_download_history') THEN
    DROP POLICY IF EXISTS "Users can view own download history" ON public.user_download_history;
    EXECUTE 'CREATE POLICY "Users can view own download history" ON public.user_download_history
    FOR SELECT USING (user_id = (SELECT auth.uid()))';

    DROP POLICY IF EXISTS "Users can insert own downloads" ON public.user_download_history;
    EXECUTE 'CREATE POLICY "Users can insert own downloads" ON public.user_download_history
    FOR INSERT WITH CHECK (user_id = (SELECT auth.uid()))';
  END IF;
END $$;

-- ============================================================================
-- PART 9: Fix RLS Policies - Stripe Subscriptions (if they exist)
-- ============================================================================
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stripe_subscriptions') THEN
    ...
    EXECUTE 'CREATE POLICY "Users can view own subscription" ON public.stripe_subscriptions
    FOR SELECT USING ((SELECT auth.uid()) = user_id)';
  END IF;
END $$;

-- ============================================================================
-- PART 9B: Fix RLS Policies - Import Jobs tables
-- ============================================================================
...
CREATE POLICY "Admins can manage import items" ON public.import_items
FOR ALL TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.users
...
CREATE POLICY "Admins can manage import projects" ON public.import_detected_projects
FOR ALL TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.users
...
-- ============================================================================
-- PART 9C: Fix RLS Policies - User Access Tiers tables
-- ============================================================================

-- design_views
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'design_views') THEN
    DROP POLICY IF EXISTS "Users can see their own views" ON public.design_views;
    EXECUTE 'CREATE POLICY "Users can see their own views" ON public.design_views
    FOR SELECT USING (user_id = (SELECT auth.uid()) OR public.is_admin())';
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_activity') THEN
    ...
    EXECUTE 'CREATE POLICY "Users can see own activity" ON public.user_activity
    FOR SELECT USING (user_id = (SELECT auth.uid()))';
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subscription_history') THEN
    ...
    EXECUTE 'CREATE POLICY "Users can see own subscription history" ON public.subscription_history
    FOR SELECT USING (user_id = (SELECT auth.uid()))';
  END IF;
END $$;

-- ============================================================================
-- PART 9D: Fix RLS Policies - Stripe Integration tables
-- ============================================================================
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payment_history') THEN
    ...
    EXECUTE 'CREATE POLICY "Users can view own payment history" ON public.payment_history
    FOR SELECT USING ((SELECT auth.uid()) = user_id)';
  END IF;
END $$;

-- ============================================================================
-- PART 10: Fix Functions - Add search_path = ''
-- ============================================================================
...
-- check_duplicate_hash - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.check_duplicate_hash(text);
...
-- find_by_source_path - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.find_by_source_path(text);
...
-- search_designs - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.search_designs(text, int, int);
...
-- get_download_stats - drop first to avoid return type conflicts
...
-- get_popular_designs - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.get_popular_designs(timestamptz, int);
...
BEGIN
  ...
  FROM public.designs d
  LEFT JOIN public.downloads dl ON dl.design_id = d.id
...
-- get_designs_by_tag - drop first to avoid conflicts
DROP FUNCTION IF EXISTS public.get_designs_by_tag(text);
...
-- validate_session_with_role - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.validate_session_with_role(text, uuid);
CREATE OR REPLACE FUNCTION public.validate_session_with_role(
...
-- cleanup_old_sessions (if exists) - must drop first due to potential return type change
...
-- cleanup_expired_sessions (if exists) - must drop first due to potential return type change
...
-- update_import_job_updated_at (if exists)
...
-- get_related_designs - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.get_related_designs(text, int);
...
DECLARE
  ...
  v_difficulty TEXT;
BEGIN
  SELECT d.id, d.categories, d.style, d.difficulty
  INTO v_design_id, v_categories, v_style, v_difficulty
  FROM public.designs d
  WHERE d.slug = p_design_slug AND d.is_public = true;
  ...
  SELECT
    ...
    (
      ...
      CASE WHEN d.difficulty = v_difficulty THEN 0.5 ELSE 0 END
    )::FLOAT as relevance_score
  FROM public.designs d
  WHERE d.is_public = true
    AND d.id != v_design_id
    AND (
      d.categories && v_categories
      OR d.style = v_style
      OR d.difficulty = v_difficulty
    )
...
-- get_import_log_reasons (if exists) - must drop first due to return type change
...
BEGIN
  RETURN QUERY
  SELECT
    il.skip_reason as reason,
    COUNT(*)::BIGINT as count
  FROM public.import_logs il
  WHERE il.job_id = p_job_id
    AND il.skip_reason IS NOT NULL
  GROUP BY il.skip_reason
  ORDER BY count DESC;
END;
$$;

-- can_user_access_design (if exists)
...
BEGIN
  SELECT access_level INTO v_design_access
  FROM public.designs
  WHERE id = design_uuid;

  IF v_design_access IS NULL OR v_design_access = 'free' THEN
    RETURN TRUE;
  END IF;
  ...
  IF v_design_access = 'premium' AND v_user_tier_level >= 1 THEN
    RETURN TRUE;
  ELSIF v_design_access = 'exclusive' AND v_user_tier_level >= 2 THEN
    RETURN TRUE;
  END IF;
...
-- check_user_download_limit - drop first to avoid return type conflicts
DROP FUNCTION IF EXISTS public.check_user_download_limit(uuid);
...
BEGIN
  ...
  IF v_tier_limit = -1 THEN
    RETURN QUERY SELECT TRUE, 0, -1, NULL::TIMESTAMPTZ;
    RETURN;
  END IF;

  SELECT COUNT(*)::INT INTO v_current_downloads
  FROM public.user_download_history
  WHERE user_id = user_uuid
    AND downloaded_at >= v_period_start;

  RETURN QUERY SELECT
    v_current_downloads < v_tier_limit,
    v_current_downloads,
    v_tier_limit,
...
-- get_user_dashboard_stats - drop first to avoid return type conflicts
...
-- update_user_timestamp (if exists)
...
-- COMMENTS
-- ============================================================================
COMMENT ON FUNCTION public.is_admin() IS 'Check if current user has admin or super_admin role. Optimized with (SELECT auth.uid()) and secure search_path.';
COMMENT ON FUNCTION public.is_super_admin() IS 'Check if current user has super_admin role. Optimized with (SELECT auth.uid()) and secure search_path.';
